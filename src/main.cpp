...
void ShowScreen() {
    ...
    if (ImPlot::BeginPlot("#Screen", ImVec2(-1, 250), ImPlotFlags_AntiAliased | ImPlotFlags_Crosshairs | ImPlotFlags_NoTitle)) {
        ...
        ImPlot::PlotLine("Ch 1", &sdata1.Data[0].x, &sdata1.Data[0].y, sdata1.Data.size(), 0, 2 * sizeof(float));
        ImPlot::PlotLine("Ch 2", &sdata2.Data[0].x, &sdata2.Data[0].y, sdata2.Data.size(), 0, 2 * sizeof(float));
        ImPlot::EndPlot();
    }
    ImGui::End();
}
...
//)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float y_data[10] = { 0,1,12,3,2,5,6,7,1,9 };
        ...
        m_Future.push_back(std::async(std::launch::async, SerialRead));
        ...
        //        ImGui::PlotLines("", values, IM_ARRAYSIZE(values), values_offset, overlay, -1.0f, 1.0f, ImVec2(100, 180.0f));
        //    }
